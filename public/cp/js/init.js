	...
	$('#datable_users').DataTable({
		...
		columns: [
			...
			{ data: "balance", searchable: false,
				render: function (data, type, row) {
					return row.balance + ' рублей';
				}
			
			},
			{ data: null, searchable: false, orderable: false,
				render: function (data, type, row) {
					if(row.is_admin) return 'Администратор';
					if(row.is_moder) return 'Модератор';
			...
			{ data: null, searchable: false, orderable: false,
				render: function (data, type, row) {
					return '<a href="/admin/user/'+ row.id +'" class="btn btn-primary btn-rounded btn-xs">Редактировать</a>';
				}
			}
		],
		"language": {
...
var admintres = function(){
	...
	$('.chatapp-chat-nicescroll-bar').slimscroll({height:'483px',size: '4px', start: 'bottom', color: '#878787',disableFadeOut : true,borderRadius:0});
