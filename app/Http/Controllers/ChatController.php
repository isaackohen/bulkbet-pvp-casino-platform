...
class ChatController extends Controller
{
    ...
    public function add_message(Request $request)
    {
        ...
        if (substr_count(strtolower($messages), '/send ')) {
            ...
            if ($with < 250) {
                return response()->json(['message' => 'Для отправки Вам необходимо сделать вывод на 250 рублей!', 'status' => 'error']);
            }
            if (floor($mes[1] * 1.05) > $this->user->balance) {
                return response()->json(['message' => 'На Вашем балансе не достаточно монет для отправки!', 'status' => 'error']);
            }
            if ($mes[1] < 20) {
                return response()->json(['message' => 'Минимальная сумма перевода 20 монет!', 'status' => 'error']);
            }
            if (! $usr) {
                return response()->json(['message' => 'Не удалось найти пользователя с таким ID', 'status' => 'error']);
            }
            if ($usr->user_id == $this->user->user_id) {
                return response()->json(['message' => 'Вы не можете отправить монеты самому себе!', 'status' => 'error']);
            }
            if (! empty($mes[1])) {
        ...
        if ($this->user->access == 'admin' || $this->user->access == 'moder') {
            ...
            if (substr_count(strtolower($messages), '/ban ')) {
                ...
                if ($usr->user_id == $this->user->user_id) {
                    return response()->json(['message' => 'Вы не можете заблокировать себя!', 'status' => 'error']);
                }
                if (! empty($mes[1])) {
            ...
            if (substr_count(strtolower($messages), '/unban')) {
                ...
                if ($usr->user_id == $this->user->user_id) {
                    return response()->json(['message' => 'Вы не можете разблокировать себя!', 'status' => 'error']);
                }
                if (! empty($userid)) {
