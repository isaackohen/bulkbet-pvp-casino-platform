...
class BattleController extends Controller
{
    ...
    public function newBet(Request $request)
    {
        if (\Cache::has('bet.user.'.$this->user->id)) {
            return response()->json(['msg' => 'Вы слишком часто делаете ставку!', 'type' => 'error']);
        }
        ...
        if (is_null($color) || $color != 'red' && $color != 'blue') {
            return response()->json(['msg' => 'Цвет не найден', 'type' => 'error']);
        }
        if (is_null($sum)) {
            return response()->json(['msg' => 'Вы ввели неверную сумму', 'type' => 'error']);
        }
        if ($sum < $this->settings->battle_min_bet) {
            return response()->json(['msg' => 'Минимальная сумма ставки '.$this->settings->battle_min_bet.' PT!', 'type' => 'error']);
        }
        if ($sum > $this->settings->battle_max_bet) {
            return response()->json(['msg' => 'Максимальная сумма ставки '.$this->settings->battle_min_bet.' PT!', 'type' => 'error']);
        }
        if ($sum > $this->user->balance) {
            return response()->json(['msg' => 'Недостаточно средств для ставки', 'type' => 'error']);
        }
        if ($this->game->status == Battle::STATUS_PRE_FINISH || $this->game->status == Battle::STATUS_FINISHED) {
            return response()->json(['msg' => 'Игра уже началась или закончилась!', 'type' => 'error']);
        }
        ...
        if ($countbets >= 3) {
            return response()->json(['msg' => 'Разрешено только 3 ставки!', 'type' => 'error']);
        }
        if ($colorCheck) {
            return response()->json(['msg' => 'Вы уже сделали ставку на другой цвет!', 'type' => 'error']);
        }
    ...
    public static function getChanceOfColor($color, $game)
    {
        ...
        if (! is_null($color)) {
            $bet = BattleBets::where('game_id', $game->id)
                ...
                ->sum('price');
            if ($bet) {
                $chance = round($bet / $game->price, 2) * 100;
            }
        }
    ...
    public function getUserInGame()
    {
        ...
        foreach ($bets->where('color', 'red') as $bet) {
            $find = 0;
            foreach ($users_red as $user) {
                if ($user == $bet->user_id) {
                    $find++;
                }
            }
            if ($find == 0) {
                $users_red[] = $bet->user_id;
            }
        }
        foreach ($bets->where('color', 'blue') as $bet) {
            $find = 0;
            foreach ($users_blue as $user) {
                if ($user == $bet->user_id) {
                    $find++;
                }
            }
            if ($find == 0) {
                $users_blue[] = $bet->user_id;
            }
        }
