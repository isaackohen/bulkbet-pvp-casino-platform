...
class PvpController extends Controller
{
    ...
    public function createRoom(Request $request)
    {
        ...
        if ($request->get('value') > $this->user->balance) {
            return response()->json(['type' => 'error', 'msg' => 'На вашем балансе не хватает монет для вырполнения дейсвия!']);
        }
        if ($count >= 3) {
            return response()->json(['type' => 'error', 'msg' => 'Вы создали максимальное количество комнат!']);
        }
        if (! $request->get('value')) {
            return response()->json(['type' => 'error', 'msg' => 'Вы забыли указать сумму ставки!']);
        }
        if ($request->get('value') <= 0) {
            return response()->json(['type' => 'error', 'msg' => 'Минимальная ставка 1 монета!']);
        }
        if ($this->user->balance <= 0) {
            return response()->json(['type' => 'error', 'msg' => 'Вам не хватает монет для совершения ставки!']);
        }
    ...
    public function joinRoom(Request $request)
    {
        ...
        $coins = $room->price;
        if ($coins > $this->user->balance) {
            return response()->json(['type' => 'error', 'msg' => 'Не достаточно средств!']);
        }
        if ($room->status == 1) {
            return response()->json(['type' => 'error', 'msg' => 'Игра #'.$room->id.' уже началась!']);
        }
        if (! $coins) {
            return response()->json(['type' => 'error', 'msg' => 'Вы забыли указать сумму ставки!']);
        }
        if ($coins <= 0) {
            return response()->json(['type' => 'error', 'msg' => 'Минимальная ставка 1 PT!']);
        }
        if ($this->user->balance <= 0) {
            return response()->json(['type' => 'error', 'msg' => 'Вам не хватает PT для совершения ставки!']);
        }
        if ($room->user1 == $this->user->id) {
            return response()->json(['type' => 'error', 'msg' => 'Вы не можете учавствовать в своей игре!']);
        }
        ...
        if (($room->from1 <= $winnerTicket) && ($room->to1 >= $winnerTicket)) {
            $winner = User::where('id', $room->user1)->first();
        }
        if (($room->from2 <= $winnerTicket) && ($room->to2 >= $winnerTicket)) {
