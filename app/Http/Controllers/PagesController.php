...
class PagesController extends Controller
{
    ...
    public function refActivate(Request $request)
    {
        ...
        if ($refcode) {
            $money = 5;
            if (strtolower($code) == strtolower($this->user->affiliate_id)) {
                return [
                'success' => false,
    ...
    public function withdraw_cancel($id)
    {
        ...
        if ($withdraw->status > 0) {
            return redirect()->route('payhistory')->with('error', 'Вы не можете отменить данный вывод!');
        }
        if ($withdraw->user_id != $this->user->id) {
            return redirect()->route('payhistory')->with('error', 'Вы не можете отменить вывод другого пользователя!');
        }
    ...
    public function result(Request $request)
    {
        ...
        if (! $ip) {
            return redirect()->route('jackpot')->with('error', 'Ошибка при проверке IP free-kassa!');
        }

        $order = $this->chechOrder($request->get('MERCHANT_ORDER_ID'), $request->get('AMOUNT'));
        if ($order['type'] == 'error') {
            return ['msg' => $order['msg'], 'type' => 'error'];
        }
        ...
        if (! $user) {
            return ['msg' => 'User not found!', 'type' => 'error'];
        }
    ...
    private function chechOrder($id, $sum)
    {
        ...
        if ($sum != $merch->sum) {
            return ['msg' => 'You paid another order!', 'type' => 'error'];
        }
        if ($merch->order_id == $id && $merch->status == 1) {
            return ['msg' => 'Order alredy paid!', 'type' => 'error'];
        }
    ...
    public function withdraw(Request $request)
    {
        ...
        if (is_null($system) || is_null($val) || is_null($wallet)) {
            return [
            'success' => false,
