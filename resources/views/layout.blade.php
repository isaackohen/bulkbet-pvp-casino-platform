...
<html lang="en">
	<head>
		...
		<link rel="stylesheet" href="{{ mix('/assets/css/notifyme.css') }}">
		<link rel="stylesheet" href="{{ mix('/assets/css/media.css') }}">
		<link rel="stylesheet" href="{{ mix('/assets/css/tooltipster.css') }}">
		<link rel="preload" href="{{ mix('/assets/js/app.js') }}" as="script">
		<script src="{{ mix('/assets/js/bootstrap.js') }}" type="text/javascript" defer></script>
		<script>
            ...
            <div class="leftside">
                ...
                <div class="scroll">
                    <div class="profile-block flex flex-wrap">
                       @guest
                        <div class="no-login">
							<script async src="https://telegram.org/js/telegram-widget.js?5" data-telegram-login="{{ config('telegram_login_auth.telegram_bot_name') }}" data-size="large" data-auth-url="{{ config('app.url') }}/auth/telegram/callback" data-radius="5" data-request-access="write"></script>
                        </div>
                        @else
                        <div class="login">
                            ...
                            <div class="balance flex flex-between flex-align-center tooltip" title="1 Монета - 1 Рубль">
                                ...
                                <span><span class="money" data-balance="{{ $u->balance }}">{{ $u->balance }}</span> <i class="fas fa-coins"></i></span>
            ...
            <div class="open" id="openChat">
                <span>></span>
                  ...
                  @if($messages != 0) @foreach($messages as $sms)
        		            <div class="msg flex flex-between" id="chatm_{{$sms['time2']}}">
        		                <div class="ava">
        		                    <div class="image" style="background: url({{$sms['avatar']}}) no-repeat center center / 100%;"></div>
        		                </div>
        		                <div class="r">
        		                    <div class="top flex flex-between">
        		                        <span onclick="var u = $(this); $('.chat-input').val(u.text() + ', '); return false;">@if($sms['access'] == 'admin')<span style="color:#ffd400;">[АМС] {{$sms['username']}}</span> @elseif($sms['access'] == 'moder') <span style="color:#70afe6;">[М] {{$sms['username']}}</span> {{$sms['username']}} @elseif($sms['access'] == 'youtuber')<span style="color:#dc7979;">[YT] {{$sms['username']}}</span> {{$sms['username']}} @else {{$sms['username']}} @endif</span>
        		                        <div class="data">
											<b>{{$sms['user_id']}}</b>
										</div>
        		                        @if(Auth::user() && (auth()->user()->access == 'admin' || auth()->user()->access == 'moder'))
        		                        <div class="delete tooltip" title="Удалить" onclick="$.chatdelet({{$sms['time2']}})">
        		                            <i class="fas fa-trash-alt"></i>
        		                        </div>
											@if($sms['access'] != 'admin' || $sms['access'] != 'moder')
												<div class="delete tooltip" title="Замутить" onclick="$.mute('{{$sms['user_id']}}','{{$sms['username']}}')">
										...
										@auth
											@if($sms['access'] != 'admin' || $sms['access'] != 'moder')
												<div class="delete tooltip" title="Перевод" onclick="$.transfer('{{$sms['user_id']}}','{{$sms['username']}}')">
