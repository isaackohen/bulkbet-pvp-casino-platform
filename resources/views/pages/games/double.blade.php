...
<div class="content">
    ...
    <div class="game-content battle double flex flex-between flex-wrap">
        <div class="top-battle">
            <div class="battle-slider flex flex-center flex-align-center bg-2">
                <div class="history">
                    <div class="scroll double-last">
                        @foreach($history as $l)
                        <a href="/fair/{{$l->hash}}"><div class="hist {{ $l->winner_color }} tooltip-right" title="{{ __('platform.double.winningnumber') }}: {{ $l->winner_num }}"></div></a>
                        @endforeach
                    </div>
                </div>
            </div>
            <div class="battle-slider bg-2 flex flex-center flex-align-center">
                <div class="left">
                    <div class="slider-svg">
                        <div class="info">
                            <div class="number" style="display:none;"></div>
                            <div class="time" id="timer3">
                                <span id="rez-numbr"><i class="far fa-clock"></i> 20</span>
                            </div>
                        </div>
                        <div class="d-content">
                            <div class="arrow"></div>
                            <div class="double-slider" id="reletteact" style="transform: rotate({{ $rotate }}deg); transition: -webkit-transform 10000ms cubic-bezier(0.32, 0.64, 0.45, 1);">
                                <img src="/assets/images/double-row.png" alt="">
                            </div>
                        </div>
                    </div>
                </div>
                   ...
                   @guest
                    <div class="bet methods-value">
                        ...
                        <div class="buttons flex flex-between">
                            <a  class="red" onclick="$.wnoty({position: 'top-right',type: 'error',message: '{{ __('platform.login') }}!'});">{{ __('platform.button.placebet') }}</a>
                            <a  class="green" onclick="$.wnoty({position: 'top-right',type: 'error',message: '{{ __('platform.login') }}!'});">{{ __('platform.button.placebet') }}</a>
                            <a  class="blue" onclick="$.wnoty({position: 'top-right',type: 'error',message: '{{ __('platform.login') }}!'});">{{ __('platform.button.placebet') }}</a>
                        </div>
                    </div>
                   @else
                    <div class="bet methods-value">
                        ...
                        <ul>
                            ...
                            <li><a  data-method="clear">{{ __('platform.button.clear') }}</a></li>
                        </ul>
                        <div class="buttons flex flex-between">
                            <a  class="red double-button-1 betButton" data-bet-type="red">{{ __('platform.button.placebet') }}</a>
                            <a  class="green double-button-2 betButton" data-bet-type="green">{{ __('platform.button.placebet') }}</a>
                            <a  class="blue double-button-3 betButton" data-bet-type="black">{{ __('platform.button.placebet') }}</a>
                        </div>
                    </div>
                    @endguest
                    <div class="hash">
                        {{ __('platform.double.roundhash') }}: <span id="hash">{{$game->hash}}</span>
                    </div>
                </div>
            </div>
        </div>
        <div class="bets flex flex-between">
            <div class="red-team team" data-type="red">
                <div class="heading flex flex-between flex-align-center">
                    <span>{{ __('platform.double.totalamount') }}: <b id="bank_red">{{ (isset($prices['red'])) ? $prices['red'] : 0 }}</b></span>
                    <span class="x">x2</span>
                </div>
                <div class="rates-content_red">
                    @foreach($bets as $bet) @if($bet->type == 'red')
                    <div class="bet flex flex-between" data-userid="{{ $bet->user_id }}">
                        <div class="left">
                            <div class="ava">
                                <div class="image" style="background: url({{ $bet->avatar }}) no-repeat center center / 100%;"></div>
                            </div>
                            <div class="username">{{ $bet->username }}</div>
                            <div class="tickets"><span>{{ __('platform.double.tickets') }}</span></div>
                            <div class="amount">{{ $bet->value }} <i class="fas fa-coins"></i></div>
                        </div>
                    </div>
                    @endif @endforeach
                </div>
            </div>
            <div class="green-team team" data-type="green">
                <div class="heading flex flex-between flex-align-center">
                    <span>{{ __('platform.double.totalamount') }}: <b id="bank_green">{{ (isset($prices['green'])) ? $prices['green'] : 0 }}</b></span>
                    <span class="x">x14</span>
                </div>
                <div class="rates-content_green">
            ...
            <div class="blue-team team" data-type="black">
                <div class="heading flex flex-between flex-align-center">
                    <span>{{ __('platform.double.totalamount') }}: <b id="bank_black">{{ (isset($prices['black'])) ? $prices['black'] : 0 }}</b></span>
                    <span class="x">x2</span>
                </div>
                <div class="rates-content_black">
